...
- **TMDB-backdrop screenshots** (`ps/tmdb_screens.ps1`, new step 9): when `use_tmdb_screens=1`, the movie/TV pipeline skips the native screenshot capture (step 3) and image-host upload (step 4) and instead lets the user pick TMDB backdrops as the screenshot set. Step 4 (`tmdb`) writes every available backdrop to the `BACKDROPS` section of `<name>_tmdb.txt` (no longer capped at 10), and the new step 9 renders them as an interactive sixel grid (`tmdb_screens_per_row` x `tmdb_screens_rows` per page) using ImageMagick + chafa, paginates with `n`/`p`, accepts numeric picks (e.g. `1 4 7` or `1,4,7` across any pages), writes the chosen URLs to `<name>_screens.txt`, and re-runs `description.ps1` to rebuild the BBCode description with those backdrops
- **Configurable screenshot count** (`ps/screens.ps1`): new `screen_count` config (default 3) controls how many screenshots step 3 captures. Timestamps are split into N equal slots across the middle 80% of the runtime (10%-90%) with a random pick within each slot, so captures stay evenly distributed and never overlap
- **Configurable screenshot image size** (`ps/description.ps1`, `templates/screenshots.bbcode`): new `screenshot_img_size` config (default 400) replaces the hardcoded `[img=400]` width. The per-image template now uses a `{{IMG_SIZE}}` placeholder substituted from config; screenshots are also always joined with a space so the rendered description has a visible gap between image links

### Improvements
- **Pipeline reorder**: screenshot upload now runs immediately after capture instead of after AI description. New step ordering: `1 parse, 2 create, 3 screens, 4 upload, 5 tmdb, 6 imdb, 7 describe, 8 description, 9 tmdb_screens`. CLI step names (`upload`, `tmdb`, `imdb`, `describe`, …) keep working transparently — only numeric `-steps` values shift. The `run.bat` interactive menu, `ps/help.ps1`, README listings, and the `use_tmdb_screens` default chain (now `1,2,5,6,7,8,9`) are all updated to the new numbering
- **Adaptive screenshot upload** (`ps/screens_upload.ps1`): replaces the hardcoded `screen01..screen03` list with a numeric glob over `<name>_screen\d+\.png`, so it picks up whatever count `screens.ps1` produced
- **`run.bat` menu reflects `use_tmdb_screens`**: the Movie/TV steps menu reads the config key and tags steps 3 and 4 as `[skipped]` when `use_tmdb_screens=1`, lists step 9 below step 8, and updates the default-Enter description to `runs 1,2,5,6,7,8,9 (skips 3 and 4)`
...
- `tmdb_screens_per_row` — TMDB-screens picker grid width (default 4, clamped 1-6)
- `tmdb_screens_rows` — TMDB-screens picker grid height per page (default 2, clamped 1-4)
- `screen_count` — number of screenshots to capture in step 3 (default 3, clamped 1-30)
- `screenshot_img_size` — width (px) used in `[img=NN]` tags for screenshot thumbnails in the description (default 400)
